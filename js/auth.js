...
function applyBypassUI(user) {
    try {
        ...
        // Avatar (iniciales)
        const iniciales = (user.nombre || 'AD').split(' ').map(n => n[0]).join('').toUpperCase().substring(0, 2);
        ...
        Object.entries(modulosNavegacion).forEach(([modulo, selector]) => {
            ...
            if (!navItem) return;
            const permitido = user.rol === 'administrador' || (user.permisos && user.permisos[modulo]);
            navItem.style.display = permitido ? 'block' : 'none';
        });
    } catch (_) { /* noop */ }
}
...
class SupabaseAuthSystem {
    ...
    redirectToLogin() {
        if (AUTH_BYPASS) {
            console.log('🟡 AUTH_BYPASS: se evita redirección a login');
            return;
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (AUTH_BYPASS && (paginasProtegidas.includes(paginaActual) || paginaActual === '')) {
        console.log('🟡 AUTH_BYPASS activo: deshabilitando login y creando sesión temporal');
        initBypassAuth();
        return;
    }
